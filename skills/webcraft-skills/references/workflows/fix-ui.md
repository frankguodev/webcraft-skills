...
- If the source is Quick / Standard Audit, keep scope restrained: fix Top Findings, Fix Order, and direct dependencies.
...
- Clickable elements need semantic mouse affordance: buttons, links, search buttons, chips, card actions, icon buttons, and custom clickable regions should use `cursor: pointer` or equivalent click affordance plus hover/active feedback that does not shift layout.
...
- First-viewport layout, search area, hero, mockup, or section spacing: recheck in a browser at least one desktop viewport and one mobile viewport, ensuring there is no overlap, misalignment, hollow placeholder, or new layout shift.
