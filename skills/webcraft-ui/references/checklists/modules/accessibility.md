...
- `Critical`: keyboard cannot complete the core path, focus is invisible, dialog focus is uncontrolled, key icon buttons lack names, or contrast makes core content unreadable.
- `Major`: keyboard, semantics, state expression, target size, or high-contrast support is clearly weak, but mouse users can still complete the task.
- `Minor`: non-core accessible names, explanatory text, redundant state cues, or target sizes need improvement.
